...
# Take resolution info 
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))

# Wr init
out = cv2.VideoWriter('faster_mobilenetv3_out_1.avi', cv2.VideoWriter_fourcc(*'DIVX'), 25, (frame_width, frame_height))
...
while(cap.isOpened()):
    ...
    if ret == True:
        ...
        # Object detection and get results
        with torch.no_grad():
            prediction = model(input_tensor)[0]

        # Applying results to the framework
        for box, label, score in zip(prediction['boxes'], prediction['labels'], prediction['scores']):
            if score > 0.5:  # threshold value
                start_x, start_y, end_x, end_y = box.cpu().numpy().astype("int")
                class_name = class_names[label]
                score_percentage = round(score.item(), 2)
                display_text = f'{class_name} ({score_percentage})'
                cv2.rectangle(frame, (start_x, start_y), (end_x, end_y), (0, 255, 0), 2)
                cv2.putText(frame, display_text, (start_x, start_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
